...
// Listar sessões do personal (com filtros)
...
// Criar sessão (personal agenda algo para o aluno)
...
// Atualizar sessão (personal)
router.put('/:sessionId', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  await dbConnect();
  const personalIdFromToken = req.user?.id;
  const { sessionId } = req.params;
  const {
    status, notes, sessionDate, tipoCompromisso,
    rotinaId, diaDeTreinoId, diaDeTreinoIdentificador,
    alunoId: alunoIdBody, pseAluno, comentarioAluno
  } = req.body;
  ...
  if (alunoIdBody) {
    const aluno = await Aluno.findOne({
      _id: new Types.ObjectId(alunoIdBody),
      trainerId: new Types.ObjectId(personalIdFromToken)
    });
    if (!aluno) return res.status(403).json({ mensagem: "Aluno não pertence a você." });
  ...
  if (Object.keys(updateData).length === 0)
    return res.status(400).json({ mensagem: "Nenhum dado para atualização." });
...
router.delete('/:sessionId', authenticateToken, async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    const result = await Sessao.deleteOne({
      _id: new Types.ObjectId(sessionId),
      personalId: new Types.ObjectId(personalIdFromToken)
    });
...
// Concluir dia (aluno) — com validação de plano/token e GRACE
router.post('/aluno/concluir-dia', authenticateAlunoToken, async (req: Request, res: Response, next: NextFunction) => {
  ...
  // Inclui dataInicio (para registrar quando começou) — e valida
  ...
  // Validação do novo campo dataInicio (seu backup já esperava isso)
  if (!dataInicio) {
  ...
  try {
    // 1) Regra de negócio: aluno em dia com plano/token?
    const status = await PlanoService.isAlunoEmDiaById(alunoId);
    if (!status.ok) {
      return res.status(409).json({
        code: 'OVER_CAPACITY',
        message: status.reason === 'INACTIVE'
          ? 'Aluno inativo — regularize o status do aluno para registrar a sessão.'
          : 'A validade do slot deste aluno expirou. Faça a renovação do plano, upgrade ou adquira tokens.',
        detail: { slotEndDate: status.slotEndDate, graceUntil: status.graceUntil },
        ctas: ['upgrade', 'tokens', 'renovacao']
      });
    }
    ...
    try {
      ...
      // Reordena: dia concluído vai para o fim da fila
      ...
      // Comparar cargas com a sessão anterior (se houver)
      ...
      if (cargas && Object.keys(cargas).length > 0) {
        try {
          const sessaoAnterior = await Sessao.findOne({
            ...
            status: 'completed'
          }).sort({ concluidaEm: -1 }).session(mongoTransactionSession);

          if (sessaoAnterior && (sessaoAnterior as any).cargasExecutadas) {
            const cargasAnteriores = (sessaoAnterior as any).cargasExecutadas;
            ...
            for (const [exercicioId, cargaAtual] of Object.entries(cargas)) {
              ...
              if (
                cargaAnterior && typeof cargaAnterior === 'string' && cargaAnterior.trim() &&
                ...
                if (!isNaN(cargaAnteriorNum) && !isNaN(cargaAtualNum) && cargaAtualNum > cargaAnteriorNum) {
                  aumentouCarga = true;
                  detalhesAumentoCarga.push({ exercicioId, cargaAnterior, cargaAtual });
                }
              }
            }
          }
      ...
      return res.status(201).json({
        message: "Dia de treino concluído com sucesso!",
        _id: novaSessao._id,
        status: novaSessao.status,
        concluidaEm: novaSessao.concluidaEm
      });
    } catch (error) {
      if (mongoTransactionSession.inTransaction()) await mongoTransactionSession.abortTransaction();
      return next(error);
...
// Feedback do aluno em uma sessão

...
import Sessao, { TIPOS_COMPROMISSO } from '../../models/Sessao.js';
...
router.get('/', authenticateToken, async (req, res, next) => {
    ...
    try {
        const queryFilter = { personalId: new Types.ObjectId(personalIdFromToken) };
        ...
        if (tipoCompromissoQuery && typeof tipoCompromissoQuery === 'string' && TIPOS_COMPROMISSO.includes(tipoCompromissoQuery)) {
        ...
        const sessoes = await query.lean();
        res.json(sessoes);
    }
    catch (error) {
...
router.post('/', authenticateToken, async (req, res, next) => {
    ...
    try {
        ...
        if (!aluno)
            return res.status(403).json({ mensagem: "Aluno não pertence a você." });
        ...
        const sessaoPopulada = await Sessao.findById(novaSessaoDoc._id).populate('alunoId', 'nome _id').populate('rotinaId', 'titulo _id').lean();
...
router.put('/:sessionId', authenticateToken, async (req, res, next) => {
    ...
    if (!Types.ObjectId.isValid(sessionId))
        return res.status(400).json({ mensagem: "ID da sessão inválido." });
    const updateData = {};
    if (status)
    ...
    if (rotinaId !== undefined)
        updateData.rotinaId = rotinaId ? new Types.ObjectId(rotinaId) : null;
    if (diaDeTreinoId !== undefined)
        updateData.diaDeTreinoId = diaDeTreinoId ? new Types.ObjectId(diaDeTreinoId) : null;
    ...
    if (pseAluno !== undefined)
        updateData.pseAluno = pseAluno;
    if (comentarioAluno !== undefined)
        updateData.comentarioAluno = comentarioAluno;
    ...
    try {
        ...
        const sessaoAtualizadaPopulada = await Sessao.findById(sessaoExistente._id).populate('alunoId', 'nome _id').populate('rotinaId', 'titulo _id').lean();
...
router.patch('/:sessionId/feedback', authenticateAlunoToken, async (req, res, next) => {
    ...
    try {
        ...
        if (pseAluno)
            sessao.pseAluno = pseAluno;
        if (comentarioAluno !== undefined)
            sessao.comentarioAluno = comentarioAluno;
        await sessao.save();

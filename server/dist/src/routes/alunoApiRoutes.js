...
router.post('/workout-log', authenticateAlunoToken, async (req, res, next) => {
    await dbConnect();
    ...
    try {
        ...
        const { treinoId, treinoTitulo, dataInicio, dataFim, duracaoTotalMinutos, nivelTreino, comentarioAluno, aumentoCarga, } = req.body;
...
router.patch('/meus-treinos/:rotinaId/cargas', authenticateAlunoToken, async (req, res, next) => { await dbConnect(); const alunoId = req.aluno?.id; const { rotinaId } = req.params; const { diaDeTreinoId, cargas } = req.body; if (!alunoId)
    ...
    let atualizacoes = 0;
    diaDeTreino.exerciciosDoDia?.forEach(exercicio => { const exercicioIdString = exercicio._id.toString(); if (cargas[exercicioIdString] && exercicio.carga !== cargas[exercicioIdString]) {

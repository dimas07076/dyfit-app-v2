...
router.get("/gerenciar", authenticateToken, async (req, res, next) => { await dbConnect(); const trainerId = req.user?.id; if (!trainerId)
...
router.get('/:alunoId/rotinas', authenticateToken, async (req, res, next) => { await dbConnect(); const trainerId = req.user?.id; const { alunoId } = req.params; if (!trainerId)
    return res.status(401).json({ erro: 'Personal não autenticado.' }); if (!mongoose.Types.ObjectId.isValid(alunoId))
    return res.status(400).json({ erro: 'ID do aluno inválido.' }); try {
    ...
    const rotinas = await Treino.find({ alunoId: new mongoose.Types.ObjectId(alunoId), criadorId: new mongoose.Types.ObjectId(trainerId), tipo: 'individual' }).select('titulo descricao criadoEm atualizadoEm').sort({ atualizadoEm: -1 });
...
router.delete("/gerenciar/:id", authenticateToken, async (req, res, next) => { await dbConnect(); const trainerId = req.user?.id; const { id } = req.params; if (!trainerId)
    ...
    if (!result)
        return res.status(404).json({ erro: "Aluno não encontrado ou sem permissão." });
...
router.get('/meus-treinos', authenticateAlunoToken, async (req, res, next) => {
    ...
    try {
        ...
        // Convertendo para objetos simples para garantir que todos os campos sejam incluídos na resposta
...
router.get('/meus-treinos/:id', authenticateAlunoToken, async (req, res, next) => {
    ...
    try {
        ...
        // Convertendo para objeto simples para garantir que todos os campos sejam incluídos na resposta
...
router.get('/minhas-sessoes-concluidas-na-semana', authenticateAlunoToken, async (req, res, next) => { await dbConnect(); const alunoId = req.aluno?.id; if (!alunoId)
    return res.status(401).json({ message: 'ID do aluno não encontrado no token.' }); try {
    const hoje = new Date();
    const inicioDaSemana = startOfWeek(hoje, { weekStartsOn: 1 });
    const fimDaSemana = endOfWeek(hoje, { weekStartsOn: 1 });
    const sessoesConcluidas = await Sessao.find({ alunoId: new Types.ObjectId(alunoId), status: 'completed', concluidaEm: { $gte: inicioDaSemana, $lte: fimDaSemana }, }).select('_id sessionDate concluidaEm tipoCompromisso').sort({ concluidaEm: 1 }).lean();
    res.status(200).json(sessoesConcluidas);
}
catch (error) {
    next(error);
} });
router.get('/meu-historico-sessoes', authenticateAlunoToken, async (req, res, next) => { await dbConnect(); const alunoId = req.aluno?.id; if (!alunoId) {
    return res.status(401).json({ message: 'ID do aluno não encontrado no token.' });
} const page = parseInt(req.query.page) || 1; const limit = parseInt(req.query.limit) || 5; const skip = (page - 1) * limit; try {
    const query = { alunoId: new Types.ObjectId(alunoId), status: 'completed' };
    const totalSessoes = await Sessao.countDocuments(query);
    const sessoes = await Sessao.find(query).sort({ concluidaEm: -1 }).skip(skip).limit(limit).populate({ path: 'rotinaId', select: 'titulo' }).lean();
    res.status(200).json({ sessoes, currentPage: page, totalPages: Math.ceil(totalSessoes / limit), totalSessoes, });
}
catch (error) {
    next(error);
} });
router.patch('/meus-treinos/:rotinaId/cargas', authenticateAlunoToken, async (req, res, next) => { await dbConnect(); const alunoId = req.aluno?.id; const { rotinaId } = req.params; const { diaDeTreinoId, cargas } = req.body; console.log('--- [PATCH /cargas] Rota acionada ---'); console.log('Dados recebidos do frontend (req.body):', JSON.stringify(req.body, null, 2)); if (!alunoId)
    return res.status(401).json({ message: "Aluno não autenticado." }); if (!mongoose.Types.ObjectId.isValid(rotinaId) || !mongoose.Types.ObjectId.isValid(diaDeTreinoId)) {
    ...
    diaDeTreino.exerciciosDoDia?.forEach(exercicio => { const exercicioIdString = exercicio._id.toString(); console.log(`Verificando ID do BD: ${exercicioIdString}. Existe em 'cargas'?`, cargas.hasOwnProperty(exercicioIdString)); if (cargas[exercicioIdString] && exercicio.carga !== cargas[exercicioIdString]) {

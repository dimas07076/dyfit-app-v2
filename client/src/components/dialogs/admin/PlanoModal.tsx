...
export function PlanoModal({ 
  ...
  const deletePlanMutation = useMutation<any, Error, string>({
    mutationFn: (personalId) => {
      return fetch(`/api/admin/personal/${personalId}/plan`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
        }
      }).then(res => {
        if (!res.ok) throw new Error('Falha ao remover o plano.');
        return res.json();
      });
    },
    onSuccess: () => {
      toast({
        title: "Sucesso!",
        description: "O plano foi removido e os alunos do personal foram inativados."
      });
      queryClient.invalidateQueries({ queryKey: ['adminPersonalTrainersList'] });
      queryClient.invalidateQueries({ queryKey: ['personalPlanStatus', personal?._id] });
      onClose();
    },
    onError: (error) => {
      toast({
  ...
  const handleRemovePlan = () => {
    if (!personal) return;
    openConfirmDialog({
      titulo: "Remover Plano do Personal",
      mensagem: `Tem certeza que deseja remover o plano de ${personal.nome}? Esta ação irá inativar o plano atual e todos os alunos associados a ele. Esta ação não pode ser desfeita.`,
      textoConfirmar: "Sim, remover plano",
      onConfirm: () => deletePlanMutation.mutate(personal._id)
    });
  };
  ...
  return (
    <>
            ...
            <div className="space-y-6">
              ...
              <div className={`border-2 p-6 rounded-lg ${personal.isExpired ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200'}`}>
                <h3 className="font-semibold text-lg mb-4 flex items-center justify-between">
                    ...
                    {!personal.isExpired && personal.planoId && (
                        <Button
                            variant="destructive"
                            size="sm"
      ...
      <ModalConfirmacao
        ...
        isLoadingConfirm={deletePlanMutation.isPending}
      />
    </>
  );
}